      ...
      <v-card>
        ...
        <v-data-table v-bind:headers="headers" v-bind:items="items" v-bind:search="search" >
          <template slot='items' slot-scope='props'>
            <td class='text-xs-center' style="color:#000 !important;">{{ props.item.consumeridid }}</td>
...
export default {
  ...
  methods: {
     init: function() {
      var url = config.cigbackendUrl + "/messageList/getSubscription";
    ...
    //删除
    remove: function() {
      var url = config.cigRestUrl+"/rest/tables/subscription/" + this.removevaule;
      store.progress.commit("start", url);
      this.$http
        .delete(url)
        .then(response => {
