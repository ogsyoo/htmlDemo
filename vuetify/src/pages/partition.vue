    ...
    <div class="cig-content">
      ...
      <v-card>
        ...
        <template slot='items' slot-scope='props'>
          <td class='text-xs-center' style="color:#000 !important;">{{ props.item.partition}}</td>
          <td class='text-xs-center'>
            <div>
              <a @click="update(props.item.partition,props.item.id)">修改</a>&nbsp;&nbsp;&nbsp;
              <!--删除弹窗-->
...
export default {
  ...
  methods: {
    init: function() {
      var url = config.cigbackendUrl + "/messageList/getPartition";
    ...
    // 新增
    add: function() {
      window.location.href = "#/addPart?type=0";
    },
    // 修改
    update: function(sendName,sendId) {
      this.$router.push({
        path: "/addPart",
        query: {
          id:sendId,
    ...
    //删除
    remove: function() {
     var url = config.cigbackendUrl+"/rest/tablesAccess/partition/" + this.removevaule;
     console.log(url);
     store.progress.commit("start", url);
      this.$http
        .delete(url)
