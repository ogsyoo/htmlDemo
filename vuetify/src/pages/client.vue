    ...
    <div class="cig-content">
      ...
      <v-card>
        ...
        <template slot='items' slot-scope='props'>
          <td class='text-xs-center' style="color:#000 !important;">{{ props.item.ip }}</td>
          <td class='text-xs-center'>{{ props.item.groupName }}</td>
          <td class='text-xs-center'>
            <div>
              <a @click="update(props.item)">修改</a>&nbsp;&nbsp;&nbsp;
               <!--删除弹窗-->
...
export default {
  ...
  methods: {
    init: function() {
      var url = config.cigbackendUrl + "/messageList/getClient";
    ...
    // 新增
    add: function() {
      window.location.href = "#/addClient?type=0";
    },
    // 修改
    ...
    // 删除
    remove: function() {
      var url = config.cigRestUrl + "/rest/tablesAccess/client/" + this.removevaule;
      store.progress.commit('start', url);
